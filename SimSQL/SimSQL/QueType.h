...
public: 
    ...
    ~QueType();
    // Class destructor.
	QueType(const QueType<ItemType>& another);
	...
	// Function: 모든 oldItem을 newItem으로 바꾼다.
	// Post: 모든 oldItem을 newItem으로 바꾼다.
...
template <class ItemType>
QueType<ItemType>::QueType(const QueType<ItemType>& another)// Class copy constructor.
// Post:  front and rear are set to NULL.
{
	if (IsFull())
		throw FullQueue();
	...
	{
		NodeType<ItemType>* location=another.front;
		while (location != NULL){
